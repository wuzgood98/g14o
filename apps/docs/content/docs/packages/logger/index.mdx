...
`@g14o/logger/verbose` provides `resolveVerboseLogger` for package diagnostics (`@g14o/cache`, `@g14o/events`, `@g14o/ratelimit`). Pass `verbose: true` for console output, or inject a custom `info` / `warn` / `error` logger.
