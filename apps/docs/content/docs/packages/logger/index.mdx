...
`@g14o/cache`, `@g14o/events`, and `@g14o/ratelimit` accept optional verbose diagnostics without a runtime dependency on `@g14o/logger`. Pass `verbose: true` for console output, or pass a duck-typed `{ info, warn, error }` adapter as `verbose`. Install `@g14o/logger` separately when you want `createLogger` for structured injection.

## Next steps

- [Formatting](/packages/logger/formatting) — pretty console, JSON, timestamps, errors, Node vs browser
- [Examples](/packages/logger/examples) — child loggers, request IDs, timing, redaction
