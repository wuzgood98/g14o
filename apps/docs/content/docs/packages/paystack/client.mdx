...
See [API reference](/packages/paystack/api) for the full `PaystackClientOptions` table.

## Payment flow

`transactions.initialize` only **starts** checkout. Always confirm payment server-side with `transactions.verify` or a [`charge.success` webhook](/packages/paystack/webhooks) before delivering value.
...
Param types: [InitializeTransactionParams](/packages/paystack/api#initializetransactionparams).
...
Param types: [ChargeAuthorizationParams](/packages/paystack/api#chargeauthorizationparams).
...
Param types: [CreateCustomerParams](/packages/paystack/api#createcustomerparams).
...
Param types: [CreatePlanParams](/packages/paystack/api#createplanparams).
...
Param types: [CreateSubscriptionParams](/packages/paystack/api#createsubscriptionparams), [DisableSubscriptionParams](/packages/paystack/api#disablesubscriptionparams).
...
Webhook-specific errors (`WebhookVerificationError`, `WEBHOOK_PROCESSING_ERROR`) are documented on the [Webhooks](/packages/paystack/webhooks) page.

See [API reference](/packages/paystack/api) for full type tables.
