...
Plain return values (non-`Result`) are cached. Bare `null` and `undefined` are not cached (`get` uses `null` for missing keys). Use a `Result` or domain value when an intentional empty answer should be cached.
