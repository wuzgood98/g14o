...
When `verbose: true`, the bus logs diagnostics to the console (`info` / `warn` / `error`). Silent by default. Pass `logger` to route diagnostics through your own `info` / `warn` / `error` adapter instead (wins over `verbose`).
...
export const event = new Event({
  ...
  stream: memoryStream(),
  verbose: true,
  // logger: appLogger, // optional injectable adapter
});
```
